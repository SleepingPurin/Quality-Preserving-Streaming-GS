...
def readCamerasFromTransforms(path, transformsfile, white_background, extension=".png", lazy_image_loading=False):
    cam_infos = []
    ...
    with open(os.path.join(path, transformsfile)) as json_file:
        ...
        for idx, frame in enumerate(frames):
            ...
            cam_infos.append(CameraInfo(uid=idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image,
                                        ...
                                        height=height, fid=frame_time, alpha_mask=mask))
...
def readNerfSyntheticInfo(
    ...
    if not skip_train:
        ...
        train_cam_infos = readCamerasFromTransforms(
            path, "transforms_train.json", white_background, extension, lazy_image_loading)
    ...
    if not skip_val:
        ...
        val_cam_infos = readCamerasFromTransforms(
            path, "transforms_val.json", white_background, extension, lazy_image_loading)
    else:
        val_cam_infos = []
    if not skip_test:
        print("Reading Test Transforms")
        test_cam_infos = readCamerasFromTransforms(
            path, "transforms_test.json", white_background, extension, lazy_image_loading)
    else:
        test_cam_infos = []
