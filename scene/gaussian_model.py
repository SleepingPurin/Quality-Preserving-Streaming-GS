...
class GaussianModel:
    ...
    def training_setup(self, training_args, fastgs_args=None):
        self.fastgs_enabled = fastgs_args is not None
        ...
        if self.fastgs_enabled:
            main_groups = [
                {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
                {'params': [self._features_dc], 'lr': fastgs_args.lowfeature_lr, "name": "f_dc"},
            ...
            sh_groups = [
                {'params': [self._features_rest], 'lr': fastgs_args.highfeature_lr / 20.0, "name": "f_rest"},
            ]
            self.optimizer = torch.optim.Adam(main_groups, lr=0.0, eps=1e-15)
            self.shoptimizer = torch.optim.Adam(sh_groups, lr=0.0, eps=1e-15)
        else:
            l = [
                {'params': [self._xyz], 'lr': training_args.position_lr_init * self.spatial_lr_scale, "name": "xyz"},
    ...
    def optimizer_step(self, iteration):
        if not self.fastgs_enabled or self.shoptimizer is None:
            self.optimizer.step()
    ...
    def densify_and_split_fastgs(self, metric_mask, filter_mask, N=2):
        ...
        mask = torch.logical_and(metric_mask, filter_mask)
    ...
    def densify_and_prune_fastgs(
        ...
        grad_qualifiers = torch.norm(grads, dim=-1) >= fastgs_args.grad_thresh
        grad_qualifiers_abs = torch.norm(grads_abs, dim=-1) >= fastgs_args.grad_abs_thresh
        clone_qualifiers = torch.max(self.get_scaling, dim=1).values <= fastgs_args.dense * extent
        split_qualifiers = torch.max(self.get_scaling, dim=1).values > fastgs_args.dense * extent
        ...
        if importance_score is None:
            metric_mask = torch.zeros_like(all_clones, dtype=torch.bool)
        else:
            metric_mask = importance_score > fastgs_args.metric_threshold
        ...
        if pruning_score is None:
            ...
            to_remove = int(torch.sum(prune_mask).item())
            remove_budget = min(
                int(fastgs_args.prune_budget_fraction * to_remove),
                scores.shape[0],
            )
            if remove_budget > 0:
                n_init_points = self.get_xyz.shape[0]
                padded_importance = torch.zeros((n_init_points), dtype=torch.float32, device=self.get_xyz.device)
                padded_importance[:scores.shape[0]] = 1.0 / (1e-6 + scores.reshape(-1))
                positive_count = int(torch.count_nonzero(padded_importance).item())
                if positive_count > 0:
                    sample_count = min(remove_budget, positive_count)
    ...
    def final_prune_fastgs(self, min_opacity, pruning_score=None):
        prune_mask = (self.get_opacity < min_opacity).squeeze()
        scores_mask = torch.zeros_like(prune_mask, dtype=torch.bool)
        if pruning_score is not None:
            scores_mask[:pruning_score.shape[0]] = pruning_score > 0.9
